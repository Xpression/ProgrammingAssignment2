...
test.cacheSolve <- function() {
  ...
  ## cacheSolve should return the inverse matrix, and cache it if not allready calculated.
  a <- matrix(c(1, -1, 1, 2), 2, 2)
  ca = makeCacheMatrix(a)
  checkEquals(solve(a), cacheSolve(ca))
  
  ## cacheSolve should return the inverse matrix from the cache if allready calculated.
  checkEquals(solve(a), cacheSolve(ca))
  
  ## if the matrix has changed, i.e. the cached copy is stale, it should be updated before
